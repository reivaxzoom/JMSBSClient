    ...
    public void setUp() {
        ...
        try {
            ...
            ConnectionFactory connectionFactory
                    = (ConnectionFactory) context.lookup("localConnectionFactory");
            Connection connection = connectionFactory.createConnection();
            connection.start();
            Session session = connection.createSession(true, Session.SESSION_TRANSACTED);
            Topic requestTopic = session.createTopic("requestTopic");
            MessageConsumer subscriber2 = session.createDurableSubscriber(requestTopic, "addidas", "category='shoes' OR category='sport' OR category= 'test'  ",true);
            MessageConsumer subscriber1 = session.createDurableSubscriber(requestTopic, "bestBuy", "category='computer' OR category='software' OR category='hardware' OR category= 'test'",true);
            MessageConsumer subscriber3 = session.createDurableSubscriber(requestTopic, "wallmart", "category='Cleaners' OR category='Personal Care' OR category='Meat' OR category= 'Beverages' OR category= 'test' ",true);
            MessageConsumer subscriber4 = session.createDurableSubscriber(requestTopic, "supermarket");

            Destination responseQueue =null;
            responseQueue = new AMQAnyDestination(properties.get("queue.responseQueue") + "; {create: always}");
...
//                Assert.assertEquals(numberSampleMessages,receivedMsg);
    ...
    public void bestBuy() {
        ...
        try {
            ...
            session = connection.createSession(true, Session.SESSION_TRANSACTED);
            Topic requestTopic = session.createTopic("requestTopic");
            MessageConsumer subscriber1 = session.createDurableSubscriber(requestTopic, "bestBuy");
            Message message;
...
//                Assert.assertEquals(numberSampleMessages,receivedMsg);
    ...
    public void wallmart() {
        ...
        try {
            ...
            session = connection.createSession(true, Session.SESSION_TRANSACTED);
            Topic requestTopic = session.createTopic("requestTopic");
            MessageConsumer subscriber1 = session.createDurableSubscriber(requestTopic, "wallmart");
            Message message;
